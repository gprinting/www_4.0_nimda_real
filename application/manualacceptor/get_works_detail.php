...
$param['request_method'] = "get_works_detail";
$order_nums = explode('|',$param['order_ids']);
    ...
    while ($rs && !$rs->EOF) {
        ...
        */
        ...
        if (in_array("COLOR_INFO", $masks)){
            ...
            } else if(strpos($rs->fields['order_num'],'EV') !== false ) {
                if(strpos($rs->fields['print_tmpt_name'],'단면1도')) {
                    $info['color_info']['front'] = "1";
                    $info['color_info']['back'] = "0";
                } else {
                    $info['color_info']['front'] = "4";
                    $info['color_info']['back'] = "0";
                }
            }
            else {
