...
$param['count'] = acceptorhelper::$CALL_WORK_COUNT_ATONCE;
$param['request_method'] = "get_work_list_manual";
$rs = $dao->selectWorkListForManual($conn, $param);
...
$inserted_onefile_order_nums = array();
...
while ($rs && !$rs->EOF) {
    ...
    if($rs->fields['onefile_etprs_yn'] == "O") {
        ...
        foreach($info['tied_order_ids'] as $order_id) {
            array_push($inserted_onefile_order_nums, $order_id);
        }
    }
    ...
    if(!in_array($info['order']['id'], $inserted_onefile_order_nums))
