...
require_once('/gpdir/gpnfs/nimda/application/libraries/FPDF/vendor/autoload.php');
require_once('/gpdir/gpnfs/nimda/application/libraries/FPDF/vendor/korean.php');
...
foreach($files as $file) {
    ...
    $cut_width = $infos[5];
    $cut_height = $infos[6];
    $gap_width = ($infos[3] - $infos[5]) / 2;
    $gap_height = ($infos[4] - $infos[6]) / 2;
    //$pdf->SetLineWidth(0.1);
    // TopLeft
    $pdf->Line($infos[1] - 2, $infos[2] + $gap_height, $infos[1] - 1, $infos[2] + $gap_height); // ㅡ
    $pdf->Line($infos[1] + $gap_width, $infos[2] - 2, $infos[1]  + $gap_width, $infos[2] - 1); // ㅣ

    // TopRight
    $pdf->Line($infos[1] + $infos[3] - $gap_width, $infos[2] - 2, $infos[1] + $infos[3] - $gap_width, $infos[2] - 1); // l
    $pdf->Line($infos[1] + $infos[3] + 1, $infos[2] + $gap_height, $infos[1] + $infos[3] + 2, $infos[2] + $gap_height); // ㅡ


    // BottomLeft
    $pdf->Line($infos[1] - 2, $infos[2] + $infos[4] - $gap_height,     $infos[1] - 1, $infos[2] + $infos[4] - $gap_height);
    $pdf->Line($infos[1] + $gap_width, $infos[2] + $infos[4] + 2, $infos[1] + $gap_width,     $infos[2] + $infos[4] + 1);

    // BottomRight
    $pdf->Line($infos[1] + $infos[3] + 2, $infos[2] + $infos[4] - $gap_height, $infos[1] + $infos[3] + 1, $infos[2] + $infos[4] - $gap_height); // ㅡ
    $pdf->Line($infos[1] + $infos[3] - $gap_width, $infos[2] + $infos[4] + 2, $infos[1] + $infos[3] - $gap_width, $infos[2] + $infos[4] + 1); // |
}
...
foreach($files as $file) {
    ...
    if($infos[7] != "0") {
        ...
        if($size['w'] > $size['h']) {
            if($infos[7] == "180")
                $pdf_tmp->AddPage('L', [$size['w'], $size['h'], 'Rotate' => $infos[7]]);
            else
                $pdf_tmp->AddPage('P', [$size['w'], $size['h'], 'Rotate' => $infos[7]]);
        }
        else
        {
            if($infos[7] == "180")
...
$pdf->AddPanPage($pan_width, $pan_height, $trim_left, $trim_top, $trim_right, $trim_bottom, 180);
...
foreach($files as $file) {
    ...
    if($infos[7] == "0" || $infos[7] == "180")
        $infos[7] = ($infos[7] + 180) % 360;
