...
function getDeliveryListByCar() {
    var url = "/ajax/produce/process_mng/delivery_list_by_car.php";
    carname = $("#kindofcar").val();
    if(carname == "")
    {
        alert("호차를 선택해 주십시요.");
        return;
    ...
    var callback = function(result) {
        ...
        var html = "";
        for (i = 0; i < length; i++) {a_obj = obj["product"][i];
            var order_state = a_obj.order_state;
            var order_state_first = order_state.substring(0, 1);
            var count = a_obj.amt + " X " + a_obj.count;
            var order_detail = a_obj.order_detail;

            html += "<li class='ui-li-static ui-body-inherit ui-first-child'>";
            html += makeHead(a_obj.title);
            html += makeDesc(a_obj.order_detail);
            html += makeDesc(count);
            html += makeDesc(a_obj.addr);
            html += "</li>";
        }
        $("#item_wait_list").html(html);
    };
...
function doConvert() {
    if (!((event.charCode >= 48 && event.charCode <= 57) || // 0-9
        ...
        order_detail_num.value = korTypeToEng(order_detail_num.value);
}
...
var cndSearch = {
    "exec"       : function() {
        ...
        var url = "/ajax/manufacture/storage_mng/load_dlvr_list.php";
...
function makeHead(str) {
    str = "<h3 class='ui-li-heading'>" + str + "</h3>";
    return str;
}

function makeDesc(str) {
    str = "<p class='ui-li-desc'>" + str + "</p>";
    return str;
}
...
function get_order_state(order_num) {
