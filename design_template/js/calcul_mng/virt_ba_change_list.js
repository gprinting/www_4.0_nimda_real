...
var loadVirtBaList = function(pg) {
    ...
    $.ajax({
        ...
        contentType : false,
        data: formData,
        url: "/ajax/calcul_mng/virt_ba_list/load_virt_ba_change_list.php",
...
//회원 가상계좌 상세
var loadVirtBaChangeDetail = function(seq) {
    ...
    $.ajax({
        ...
        url: "/ajax/calcul_mng/virt_ba_list/load_virt_ba_change_detail.php",
        success: function(result) {
            openRegiPopup($("#ba_popup").html(), 650);
            var arr = JSON.parse(result);
            $("#before_bank_name").val(arr.bank_name);
            $("#before_bank_account").val(arr.ba_num);
            $("#after_bank_name").val(arr.bank_aft);
            $("#virt_ba_change_history_seqno").val(seq);
            $("#member_seqno").val(arr.member_seqno);
            $("#depo_name").val(arr.depo_name);
            //$("#bank_name").html(result);
        }
    });
