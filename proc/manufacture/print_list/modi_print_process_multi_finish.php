...
if($fb->form("typset_num")) {
    $sheet_typset_seqno_arr = array();
    $typset_num = $fb->form("typset_num");
    $typset_num_length = strlen($typset_num);

    $typset_seqno = $OutputDAO->selectSeqnoByTypsetNum($conn, $typset_num);

    array_push($sheet_typset_seqno_arr, $typset_seqno);
}
...
foreach($sheet_typset_seqno_arr as $sheet_typset_seqno) {
    ...
    $rs = $OutputDAO->selectOrderNumInTypsetForPrint($conn, $sheet_typset_seqno);
    $after_list = array();
    while ($rs && !$rs->EOF) {
        $param['ordernum'] = $rs->fields['order_num'];
        $is_envelope = strpos($param['ordernum'], "EV");
        if(($rs->fields['before_cutting_yn'] == "Y" || $is_envelope) && $state == "2420") {
            $param['state'] = $state;
            ...
            $param['state'] = "2620";
            $has_after = true;
            if(!in_array($rs->fields['after_name'],$after_list)) {
                array_push($after_list, $rs->fields['after_name']);
            }
        }
        else
            $param['state'] = $state;
    ...
    if($has_after)
        ...
        $param['state'] = $state;
    $param['detail'] = implode(",",$after_list);
