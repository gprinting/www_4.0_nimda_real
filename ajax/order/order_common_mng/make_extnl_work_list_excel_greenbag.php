...
$objPHPExcel = PHPExcel_IOFactory::load($base_path . "extnl_greenbag_template.xlsx");
...
while ($rs && !$rs->EOF) {
    ...
    $type = explode(' / ', $rs->fields['order_detail'])[2];
    ...
    $sheet->setCellValue("B" . $i, $stan_name); // 상단날짜
    $sheet->setCellValue("C" . $i, $title); // 상단날짜
    $sheet->setCellValue("D" . $i, $cut_size_wid . "x" . $cut_size_vert); // 상단날짜
    $sheet->setCellValue("E" . $i, $amt); // 상단날짜
    $sheet->setCellValue("F" . $i, $tmpt); // 상단날짜
    $sheet->setCellValue("G" . $i, ""); // 상단날짜
    $sheet->setCellValue("H" . $i, $name); // 상단날짜
    $sheet->setCellValue("I" . $i, $tel_num); // 상단날짜
    $sheet->setCellValue("J" . $i, $cell_num); // 상단날짜
    $sheet->setCellValue("K" . $i, '(' . $zipcode . ')' . $address); // 상단날짜
    $sheet->setCellValue("L" . $i, $dlvr_way); // 상단날짜
    $sheet->setCellValue("Q" . $i, "디프린팅"); // 상단날짜
    $sheet->setCellValue("S" . $i++, "02)2260-9014"); // 상단날짜
...
array_push($files, [
    "path" => $path,
    "name" => $save_name. ".xlsx",
    "save_name" => date('md') . ' 그린백 제작의뢰서.xlsx'
]);
...
if (count($files) > 1) {
    ...
    $down_file_name = date('md') . ' 그린백 제작의뢰서.zip';
} else {
