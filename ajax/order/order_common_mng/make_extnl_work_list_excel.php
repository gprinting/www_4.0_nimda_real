...
if($kind == "카드명함")
    $objPHPExcel = PHPExcel_IOFactory::load($base_path . "extnl_card_template.xlsx");

if($kind == "그린백")
    $objPHPExcel = PHPExcel_IOFactory::load($base_path . "extnl_greenbag_template.xlsx");

if($kind == "배너")
    $objPHPExcel = PHPExcel_IOFactory::load($base_path . "extnl_banner_template.xlsx");
...
if($kind == "카드명함") {
    ...
    while ($rs && !$rs->EOF) {
        ...
        $file_name = date('md') . '-' .
            $member_name . '-' .
            explode(' ', $paper)[0] . str_replace(explode(' ', $paper)[1],'카드','') . '-' .
            $tot_tmpt . '도-' .
            $amt . '매-' .
            $count . '건' .
            ".pdf";

        if ($cate_sortcode == "003002001") {
            ...
            $file_name = date('md') . '-' .
                ...
                $title . '-' .
                $after_detail . '-' .
                explode(' ', $paper)[0] . explode(' ', $paper)[1] . '-' .
                $tot_tmpt . '도-' .
                $amt . '매' .
                $count . '건' .
                ".pdf";
        ...
        $sheet->setCellValue("D" . $i, $tot_tmpt); // 상단날짜
        $sheet->setCellValue("E" . $i++, $str_amt); // 상단날짜
...
array_push($files, [
    "path" => $path,
    "name" => $save_name. ".xlsx",
    "save_name" => date('md') . ' 카드명함 제작의뢰서.xlsx'
]);
...
if (count($files) > 1) {
    ...
    $down_file_name = date('md') . ' 카드명함 제작의뢰서.zip';
} else {
