...
function makeTopExcelSheet($param, $sheet, $styleArray) {
    ...
    $sheet->setCellValue('C'.$i, '거래처');
    $sheet->setCellValue('D'.$i, '인쇄제목');
    $sheet->setCellValue('E'.$i, '종이');
    $sheet->setCellValue('F'.$i, '사이즈');
    $sheet->setCellValue('G'.$i, '도수');
    $sheet->setCellValue('H'.$i, '수량');
    $sheet->setCellValue('I'.$i, '후가공');
    $sheet->setCellValue('J'.$i, '메모');
    $sheet->setCellValue('K'.$i, '인쇄');
    //가운데 정렬(행,열)
...
function createExcelInner($rs, $sheet, $styleArray) {
    ...
    while($rs && !$rs->EOF) {
        ...
        $sheet->setCellValue('C'.$i, $fields["member_name"]);
        $sheet->setCellValue('D'.$i, $fields["title"]);
        $sheet->setCellValue('E'.$i, $paper_name);
        $sheet->setCellValue('F'.$i, $fields["stan_name"]);
        $sheet->setCellValue('G'.$i, $fields["print_amt"] . $fields["print_amt_unit"]);
        $sheet->setCellValue('H'.$i, ($fields["beforeside_tmpt"] + $fields["aftside_tmpt"]) . "도");
        $sheet->setCellValue('I'.$i, $fields["after_name"]);
        $sheet->setCellValue('J'.$i, $fields["memo"]);
        $sheet->setCellValue('K'.$i, $fields["print_etprs"]);
        //테두리 설정
