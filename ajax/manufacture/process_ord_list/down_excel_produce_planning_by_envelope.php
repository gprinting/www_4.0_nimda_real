...
function makeTopExcelSheet($param, $sheet, $styleArray) {
    ...
    $sheet->setCellValue('F'.$i, '제품');
    $sheet->setCellValue('G'.$i, '종이');
    $sheet->setCellValue('H'.$i, '사이즈');
    $sheet->setCellValue('I'.$i, '수량');
    $sheet->setCellValue('J'.$i, '건수');
    $sheet->setCellValue('K'.$i, '양테');
    $sheet->setCellValue('L'.$i, '판유형');
    //가운데 정렬(행,열)
...
function makePopSpcExcelSheet($sheet_name, $sheet, $styleArray, $styleColorArr) {
    ...
    //색상처리
    $sheet->getStyle('A'.$i.':L'.$i)->applyFromArray($styleColorArr);
...
function createExcelInner($rs, $sheet, $styleArray) {
    ...
    while($rs && !$rs->EOF) {
        ...
        $sheet->setCellValue('C'.$i, $dlvr_way);
        $sheet->setCellValue('D'.$i, $fields["member_name"]);
        $sheet->setCellValue('E'.$i, $fields["title"]);
        $sheet->setCellValue('F'.$i, explode(" / ", $rs->fields["order_detail"])[0]);
        $sheet->setCellValue('G'.$i, $paper_name);
        $sheet->setCellValue('H'.$i, $stan);
        $sheet->setCellValue('I'.$i, number_format($rs->fields["amt"]) . "매");
        $sheet->setCellValue('J'.$i, number_format($rs->fields["count"]) . "건");
        $sheet->setCellValue('K'.$i, ($rs->fields["after_name"] == "양면테입") ? "O" : "");
        $sheet->setCellValue('L'.$i, "서울");
        //테두리 설정
        $sheet->getStyle('A'.$i.':L'.$i)->applyFromArray($styleArray);
        //가운데 정렬
        $sheet->getStyle('A'.$i.':L'.$i)
