...
while($rs && !$rs->EOF) {
    ...
    $total_card_direct_pay_price += $rs->fields["card_depo_price1"]; // 조정 입금
    ...
    $input_param[$z]["cellnum"] = $fields["cell_num"]; // 업체전화번호
    $input_param[$z]["telnum"] = $fields["tel_num"]; // 업체전화번호
...
function makeTopExcelSheet($param, $sheet, $styleArray) {
    ...
    $sheet->setCellValue('G'.$i, '매출조정');
    $sheet->setCellValue('H'.$i, '입금조정');
    $sheet->setCellValue('I'.$i, '에누리');
    $sheet->setCellValue('J'.$i, '순매출');
    $sheet->setCellValue('K'.$i, '충전금(카드)');
    $sheet->setCellValue('L'.$i, '충전금(가상계좌)');
    $sheet->setCellValue('M'.$i, '충전금(현금)');
    $sheet->setCellValue('N'.$i, '현금영수증');
    $sheet->setCellValue('O'.$i, '세금계산서선발행');
    $sheet->setCellValue('P'.$i, '가상계좌(미발행요청)');
    $sheet->setCellValue('Q'.$i, '카드건별결제');
    $sheet->setCellValue('R'.$i, '카드(단말기)');
    $sheet->setCellValue('S'.$i, '현금(미발행)');
    $sheet->setCellValue('T'.$i, '발급금액');
    //테두리 설정
...
function makePopSpcExcelSheet($sheet_name, $sheet, $styleArray, $styleColorArr) {
    ...
    $sheet->setCellValue('D'.$i, '사업자상호');
    $sheet->setCellValue('E'.$i, '사업자번호');
    $sheet->setCellValue('F'.$i, '휴대폰');
    $sheet->setCellValue('G'.$i, '유선전화');
    ...
    $sheet->setCellValue('L'.$i, '충전금(가상계좌)');
    $sheet->setCellValue('M'.$i, '충전금(현금)');
    $sheet->setCellValue('N'.$i, '기타(입금조정)');
    $sheet->setCellValue('O'.$i, '현금영수증');
    $sheet->setCellValue('P'.$i, '세금계산서선발행');
    $sheet->setCellValue('Q'.$i, '가상계좌(미발행요청)');
    $sheet->setCellValue('R'.$i, '카드건별결제');
    $sheet->setCellValue('S'.$i, '카드(단말기)');
    $sheet->setCellValue('T'.$i, '현금(미발행)');
    $sheet->setCellValue('U'.$i, '발급금액');
...
function createExcelInner($rs, $sheet, $styleArray) {
    ...
    foreach($rs as $fields) {
        ...
        $sheet->setCellValue('F'.$i, $fields["cellnum"]);
        $sheet->setCellValue('G'.$i, $fields["telnum"]);
        $sheet->setCellValue('H'.$i, $fields["all_pay_price"]);
        $sheet->setCellValue('I'.$i, $fields["enuri"]);
        $sheet->setCellValue('J'.$i, $fields["pure_all_pay_price"]);
        $sheet->setCellValue('K'.$i, $fields["card_depo_price"]);
        $sheet->setCellValue('L'.$i, $fields["depo_price"]);
        $sheet->setCellValue('M'.$i, $fields["adjust_deposit"]);
        $sheet->setCellValue('N'.$i, $fields["adjust_etc"]);
        $sheet->setCellValue('O'.$i, $fields["total_cash_receipt_pay_price"]);
        $sheet->setCellValue('P'.$i, $fields["total_account_issued_pay_price"]);
        $sheet->setCellValue('Q'.$i, $fields["total_account_no_issue_pay_price"]);
        $sheet->setCellValue('R'.$i, $fields["total_card_pay_price"]);
        $sheet->setCellValue('S'.$i, $fields["total_card_direct_pay_price"]);
        $sheet->setCellValue('T'.$i, $fields["cash_price_no_issue"]);
        $sheet->setCellValue('U'.$i, $fields["issue_price"]);
