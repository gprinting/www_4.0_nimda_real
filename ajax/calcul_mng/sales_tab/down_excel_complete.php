...
while($rs && !$rs->EOF) {
    ...
    $input_param[$z]["no"] = $i; // 신청일자
    $input_param[$z]["issued_date"] = $rs->fields["sell_channel"]; // 발급일자
    $input_param[$z]["member_name"] = $rs->fields["member_name"]; // 업체명
    $input_param[$z]["crn"] = $rs->fields["crn"]; // 사업자 상호
    ...
    $input_param[$z++]["object_price"] = ($object_price);  // 정식회원명
