...
if ($ordRes == "ERROR") {
    ...
    $amtRes = $CDiv->getDivAmtOrderDetailSheetMergeDataValue(_DB_SERVER, $ordRes);

    if (is_array($amtRes)) {
        $delRes = $CDiv->setDivAmtOrderDetailSheetDataDeleteComplete(_DB_SERVER, $ordRes);

        if ($delRes == "SUCCESS") {
            $insRes = $CDiv->setDivAmtOrderDetailSheetMergeDataInsertComplete(_DB_SERVER, $amtRes, $ordRes);
